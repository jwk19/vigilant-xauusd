    ...
    <div className="panel space-y-4">
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col gap-1.5 items-end">
          {activeSessions.length === 0 && <span className="session-badge" style={{background:"rgba(107,122,153,0.15)",color:"var(--text-muted)"}}>No active session</span>}
      ...
      {activeKZ
        ? <div className="killzone-bar"><p className="text-xs font-mono text-[var(--gold-primary)] uppercase tracking-widest">⚡ {activeKZ.name} Active</p><p className="text-sm text-[var(--text-primary)] mt-0.5">{activeKZ.tip}</p></div>
        ...
        {SESSIONS.map(s => {
          ...
          return (
            <div key={s.name} className="rounded-lg px-3 py-2.5 flex flex-col gap-1"
              ...
              <p className="text-[10px] font-mono text-[var(--text-muted)]">{String(s.openHour).padStart(2,"0")}:00 – {s.closeHour===24?"00:00":String(s.closeHour).padStart(2,"0")+":00"} EAT</p>
            </div>
          );
        })}
      </div>
          ...
          {KILL_ZONES.map(kz => {
            ...
            return (
              <div key={kz.name} className="flex items-center justify-between rounded px-2.5 py-1.5 text-xs"
                ...
                <span className="font-mono text-[var(--text-muted)]">{String(kz.openHour).padStart(2,"0")}:{String(kz.openMin).padStart(2,"0")} – {String(kz.closeHour).padStart(2,"0")}:{String(kz.closeMin).padStart(2,"0")}</span>
              </div>
            );
          })}
        </div>
